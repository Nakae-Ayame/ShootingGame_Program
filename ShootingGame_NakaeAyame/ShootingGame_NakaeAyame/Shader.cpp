...
void Shader::Create(std::string vs, std::string ps)
{
	...
	// 頂点シェーダーオブジェクトを生成、同時に頂点レイアウトも生成
								//
	bool sts = CreateVertexShader(device,vs.c_str(),"vs_main","vs_5_0",layout,numElements,&m_pVertexShader,&m_pVertexLayout);
	if (!sts)
	{
	...
	// ピクセルシェーダーオブジェクトを生成
	sts = CreatePixelShader(device,ps.c_str(),"ps_main","ps_5_0",&m_pPixelShader);
	if (!sts)
	{
